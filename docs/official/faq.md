...
### What have been the major influences in the language’s design? [#](#what-have-been-the-major-influences-in-the-languages-design)
...
### How is Odin licensed? [#](#how-is-odin-licensed)

The Odin compiler and the library are under the [zlib license](https://github.com/odin-lang/Odin/blob/master/LICENSE).

### Does Odin have any third-party library? [#](#does-odin-have-any-third-party-library)

Check out a few selected libraries at <https://github.com/odin-lang/Odin/wiki/Odin-Libs>.

### What are the guiding principles behind the design of Odin? [#](#what-are-the-guiding-principles-behind-the-design-of-odin)
...
### What does Odin offer over other languages? [#](#what-does-odin-offer-over-other-languages)
...
### Why does Odin not have feature X? [#](#why-does-odin-not-have-feature-x)
...
### Why does Odin not have exceptions? [#](#why-does-odin-not-have-exceptions)
...
### Is Odin “just” a language for game development? [#](#is-odin-just-a-language-for-game-development)
...
### Is Odin an object oriented language? [#](#is-odin-an-object-oriented-language)
...
### Why does Odin not have Uniform Function Call Syntax (UFCS)? [#](#why-does-odin-not-have-uniform-function-call-syntax-ufcs)
...
### Why does Odin have explicit procedure overloading but not implicit procedure overloading? [#](#why-does-odin-have-explicit-procedure-overloading-but-not-implicit-procedure-overloading)
...
### Why does Odin not have operator overloading? [#](#why-does-odin-not-have-operator-overloading)
...
### What does `distinct` do? [#](#what-does-distinct-do)
...
### What is the type of `x` in `x := 123`? [#](#what-is-the-type-of-x-in-x--123)
...
### What is the size of `int`? [#](#what-is-the-size-of-int)
...
### Which of `f32` and `f64` should I prefer for floating-point mathematics? [#](#which-of-f32-and-f64-should-i-prefer-for-floating-point-mathematics)
...
### Why does Odin not have implicit numeric type conversions? [#](#why-does-odin-not-have-implicit-numeric-type-conversions)
...
### Why are slices, dynamic arrays, and maps references whilst arrays are values? [#](#why-are-slices-dynamic-arrays-and-maps-references-whilst-arrays-are-values)
...
### Why can I not index/address a constant with a variable? [#](#why-can-i-not-indexaddress-a-constant-with-a-variable)
...
### Why is it named `proc`? [#](#why-is-it-named-proc)
...
### Why does Odin not have any methods? [#](#why-does-odin-not-have-any-methods)

We believe that data and code should be separate concepts; data should not have “behaviour”.

Use a procedure.

### But really, why does Odin not have any methods? [#](#but-really-why-does-odin-not-have-any-methods)
...
### How do I define a procedure with a different calling convention? [#](#how-do-i-define-a-procedure-with-a-different-calling-convention)
...
### Does Odin have closures? [#](#does-odin-have-closures)
...
### What is the context system for? [#](#what-is-the-context-system-for)
...
### When are procedure parameters passed by value? [#](#when-are-procedure-parameters-passed-by-value)
...
### What is the difference between `new` and `make`? [#](#what-is-the-difference-between-new-and-make)
...
### What is the difference between `free` and `delete`? [#](#what-is-the-difference-between-free-and-delete)
...
### How do I create a multiple file package? [#](#how-do-i-create-a-multiple-file-package)
...
### Why isn’t X in the core library? [#](#why-isnt-x-in-the-core-library)
...
### Why are package declarations required in every file? [#](#why-are-package-declarations-required-in-every-file)
...
### Is there an official Odin package manager? [#](#is-there-an-official-odin-package-manager)

Odin will **never** officially support a package manager.

#### How do I manage my code without a “package manager”? [#](#how-do-i-manage-my-code-without-a-package-manager)
...
### What is the criteria for selection what packages go into the `vendor` library collection? [#](#what-is-the-criteria-for-selection-what-packages-go-into-the-vendor-library-collection)
...
If “taxonomization” is **absolutely** necessary, please prefer to prefix the identifiers of the declarations. Typing is not the bottleneck when programming, reading/scanning is.
...
### What does `::` mean? [#](#what-does--mean-1)
...
### Why does Odin not use keywords to prefix declarations? [#](#why-does-odin-not-use-keywords-to-prefix-declarations)
...
### Why are there two ways to do type conversions? [#](#why-are-there-two-ways-to-do-type-conversions)
...
### Why curly brackets? [#](#why-curly-brackets)
...
### Why do slice expressions use `:` and not the range syntax? [#](#why-do-slice-expressions-use--and-not-the-range-syntax)
...
### Why does Odin not have a `while` statement? [#](#why-does-odin-not-have-a-while-statement)
...
### Convert an integer to a string or vice versa? [#](#convert-an-integer-to-a-string-or-vice-versa)
...
### What does the compiler use? [#](#what-does-the-compiler-use)
...
## Changes from C/C++ [#](#changes-from-cc)
...
### Why are there no `++` or `--` operators? [#](#why-are-there-no--or----operators)
...
### Does Odin have C++-style constructors? [#](#does-odin-have-c-style-constructors)
...
### Does Odin have C++-style copy constructors? [#](#does-odin-have-c-style-copy-constructors)

No. All copies are byte-for-byte copies.

### Does Odin have C++-style move constructors? [#](#does-odin-have-c-style-move-constructors)

No. There are no ownership semantics in Odin.

### Does Odin have C++-style destructors? [#](#does-odin-have-c-style-destructors)
...
### Why is `#partial switch` needed when there is a catch-all `case:`? [#](#why-is-partial-switch-needed-when-there-is-a-catch-all-case)
...
### Why does `#reverse` not work on ranges in `for in` loops? [#](#why-does-reverse-not-work-on-ranges-in-for-in-loops)
...
#### Deferred Attributes [#](#deferred-attributes)
...
|               |                                                                                   |
| ------------- | --------------------------------------------------------------------------------- |
| deferred_none | the deferred procedure takes _none_ of the parameters from the original procedure |
| deferred_in   | the deferred procedure takes the _input_ parameters from the original procedure   |
| deferred_out  | the deferred procedure takes the _return_ values from the original procedure      |
...
### What architectures does Odin support? [#](#what-architectures-does-odin-support)
...
### Why does Odin require the MSVC toolchain on Windows? [#](#why-does-odin-require-the-msvc-toolchain-on-windows)
...
>
> Most people appear to want MinGW just to minimize the amount of stuff that they require to download, but in short, you are going to need to download the Windows SDK (and other libraries) any way. This is the problem of developing on Windows-you effectively need to use Microsoft’s toolchain.
>
...
### Does Odin Support “Embedded Programming”? [#](#does-odin-support-embedded-programming)
...
### If Odin uses LLVM, why doesn’t it support every platform LLVM supports? [#](#if-odin-uses-llvm-why-doesnt-it-support-every-platform-llvm-supports)
...
### Does the Odin compiler implement Return Value Optimization (RVO)? [#](#does-the-odin-compiler-implement-return-value-optimization-rvo)
...
### Is the Odin compiler self hosted? [#](#is-the-odin-compiler-self-hosted)
...
### Where is the Odin roadmap? [#](#where-is-the-odin-roadmap)
...
> Source: https://odin-lang.org/docs/faq
