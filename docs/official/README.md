# docs/official/ - Odin official documentation (MIT-style)
...
`docs/official/*.md` (and `docs/official/awesome-odin.md`) are produced by
`_Helpers/scrape-official.py` (sitemap + crawl + static fallback). Re-running is
idempotent - already-scraped files are skipped unless you pass `--force`.

```bash
python _Helpers/scrape-official.py            # re-entrant: skip existing
python _Helpers/scrape-official.py --force    # force re-write
```
