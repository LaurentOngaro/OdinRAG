...
description: "How to keep personal history (devlogs, project code, raw research, copyrighted scrapes) under git version control inside a public-knowledge-base repo, with no risk of accidental push to the public remote."
tags: [OdinRAG, doc/how-to, git, public-safety, workflow]
type: how-to
status: stable
date: 2026-07-01
---

# Mixing public and private history in a single repo
...
- personal daily notes in `planning/daily/`
- one-shot raw research in `_Raw/`
- scraped content under fair-use restrictions in `odin-knowledge-base/courses/` and `docs/karl_zylinski/odin-book/`
...
A `pre-push` hook is the only thing standing between a wrong `git push` and a leaked history. The hook refuses to push `main` to the public remote. The audit script `_Helpers/audit_public_safety.py` is extended to scan the `public` branch's tree, not just the working tree.
...
- The audit script `_Helpers/audit_public_safety.py` already present and passing on the current working tree.
...
```gitignore
/code/projects/*/   # except _TEMPLATE_/
/planning/daily/
/_Raw/
/odin-knowledge-base/courses/
/docs/karl_zylinski/odin-book/**/*.md
```

Replace them with a single pointer comment so future readers know where to look:

```gitignore
# Personal paths are NOT ignored at the repo level: see docs/how-to/MIXING_PUBLIC_AND_PRIVATE_HISTORY.md
# Collaborators who want to ignore them locally should copy the patterns above into .git/info/exclude.
```

The patterns above match the ones already enforced by `_Helpers/audit_public_safety.py`. Keep both in sync.
...
```bash
git add -f code/projects/ planning/daily/ _Raw/ odin-knowledge-base/courses/ docs/karl_zylinski/odin-book/
git status
...
# The exclude regex matches code/projects/<name>/ for any <name> that is not _TEMPLATE_.
git config split.public.path .
git config split.public.exclude 'code/projects/[^_]|code/projects/_[^T]|code/projects/_T[^E]|planning/daily|_Raw|odin-knowledge-base/courses|docs/karl_zylinski/odin-book'
...
# Anchored at repo root. Excludes code/projects/_TEMPLATE/.
personal_pattern='^(code/projects/(?!_TEMPLATE_/)|planning/daily|_Raw|odin-knowledge-base/courses|docs/karl_zylinski/odin-book)/'
...
## Step 6 - extend `_Helpers/audit_public_safety.py`
...
5. Treat the leaked content as compromised: rotate any secrets present, request cache scrubs where possible, and document the incident in your `planning/daily/`.
...
- `_Helpers/audit_public_safety.py` - the safety gate, extended in step 6.
- `_Helpers/docs/PUBLIC_RELEASE_CHECKLIST.md` - the pre-push procedure this guide builds on.
