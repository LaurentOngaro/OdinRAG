...
### Refactoring (last: REFA02)

- [ ] REFA02 - `_tools/jetbrains_configurations/` is currently NOT synced (per-project IDE files).
  - If we want to mutualise them, add a recursive copy step in `sync-tooling.ps1` and a `_tools/jetbrains_configurations/.template/` directory.
...
- [x] REFA01: mutualise `_tools/install_full_env.ps1` (2026-07-26)
  ...
  - stored as `templates/_tools/install_full_env.ps1.template` in `_shared/odin_tooling/`
  - `sync-tooling.ps1` now seeds it (NOT overwrites) on first sync of a new project
- [x] DOC02: tasks/launch wiring refactor (2026-07-26)
