...
**Note:** Declarations have to be _unique_ within a scope.
...
### Organizing packages [#](#organizing-packages)
...
where `a..=b` denotes a closed interval `[a,b]`, i.e. the upper limit is _inclusive_, and `a..<b` denotes a half-open interval `[a,b)`, i.e. the upper limit is _exclusive_.
...
The iterated values are _copies_ and cannot be written to.
...
#### `for` reverse iteration [#](#for-reverse-iteration)
...
#### `for` loop unrolling [#](#for-loop-unrolling)
...
As of the `dev-2023-07` release, mixing named and positional arguments is allowed. This is often useful when a procedure has a lot of arguments _or_ you want to customize [default values](#default-values).

**_Positional_** arguments are not allowed after **_named_** arguments.
...
### Explicit procedure overloading [#](#explicit-procedure-overloading)
...
#### Rationale behind explicit overloading [#](#rationale-behind-explicit-overloading)
...
#### Transmute operator [#](#transmute-operator)
...
### Auto cast operation [#](#auto-cast-operation)
...
Iteration through runes is preferred since odin strings are **_UTF8_**. Most core library procedures will be addressed by `*_byte` if they do input/output an index in _byte_.

#### `string` format printing [#](#string-format-printing)
...
Here is a short list of possible type conversions - including whether they _copy_ or _alias_. This is important to understand since **_Odin_** gives you the possibility to keep _allocations_ to a low degree.

If some cases are missing please let us know.

Legend:

- **_copy_** = get a freshly allocated copy of the ‘from’ data
- **_alias_** = reuse the ‘from’ data, without allocation
- **_stream_** = get individual values from the string, without allocation
- **_st_** = the input string

#### From `string` to X [#](#from-string-to-x)
...
#### From `cstring` to X [#](#from-cstring-to-x)
...
#### From a **_string literal_** to X [#](#from-a-string-literal-to-x)
...
#### From `[]rune` to `string` [#](#from-rune-to-string)

| Action | Code                       |
| ------ | -------------------------- |
| copy   | `utf8.runes_to_string(st)` |

#### From `[^]u8` to `cstring` [#](#from-u8-to-cstring)

| Action | Code          |
| ------ | ------------- |
| alias  | `cstring(st)` |

#### From `[^]u8` and length `int` to `string` [#](#from-u8-and-length-int-to-string)
...
### Compound binary operator and assign [#](#compound-binary-operator-and-assign)
...
For unsigned integers, the operations `+`, `-`, `*`, and `<<` are computed modulo 2n, where _n_ is the bit width of the unsigned integer’s type. In a sense, these unsigned integer operations discard the high bits upon overflow, and programs may rely on “wrap around”.
...
##### Swizzle Operations [#](#swizzle-operations)
...
#### Slice shorthand [#](#slice-shorthand)
...
#### Appending to a dynamic array [#](#appending-to-a-dynamic-array)
...
#### Inject / Assign to a dynamic array [#](#inject--assign-to-a-dynamic-array)
...
#### Removing from a dynamic array [#](#removing-from-a-dynamic-array)
...
- [`unordered_remove`](https://pkg.odin-lang.org/base/builtin/#unordered_remove) removes an element at a specific index. _Unordered_ means it is `O(1)`, since it swaps the last element to the removed location - making the array be _sorted_ differently.
- [`ordered_remove`](https://pkg.odin-lang.org/base/builtin/#ordered_remove) removes an element at a specific index. _Ordered_ means it will move all elements after the index downwards with a `copy` - ensuring elements remain in the same order.
...
#### Slice & Sort a dynamic array [#](#slice--sort-a-dynamic-array)
...
#### Making and deleting slices and dynamic arrays [#](#making-and-deleting-slices-and-dynamic-arrays)
...
#### Clearing a dynamic array [#](#clearing-a-dynamic-array)
...
#### Resize / Reserve with a dynamic array [#](#resize--reserve-with-a-dynamic-array)
...
### `[dynamic; N]T` fixed capacity dynamic array [#](#dynamic-nt-fixed-capacity-dynamic-array)
...
#### Implicit Selector Expression [#](#implicit-selector-expression)

An _implicit selector expression_ is an abbreviated way to access a member of an enumeration, in a context where type inference can determine the implied type. It has the following form:
...
#### Iterating an Enumeration [#](#iterating-an-enumeration)

Enums can be trivially `for` looped in odin. This way we can loop through the entire `enum` and do things like printing or inserting into an _Enumerated Array_.
...
### Enumerated Array [#](#enumerated-array)
...
The `#partial` directive can be used to initialize an enumerated array _partially_.
...
#### Type switch statement [#](#type-switch-statement)
...
#### Map Container Calls [#](#map-container-calls)
...
A `bit_field` is a record type in Odin that is akin to a bit-packed struct. **Note:** `bit_field` is **not** equivalent to `bit_set` as it has different semantics and use cases. `bit_field` fields are accessed by using a dot:
...
#### SOA Struct Arrays [#](#soa-struct-arrays)
...
#### SOA Struct Slices and Dynamic Arrays [#](#soa-struct-slices-and-dynamic-arrays)
...
#### `soa_zip` and `soa_unzip` [#](#soa_zip-and-soa_unzip)
...
#### Submatrix Casting [#](#submatrix-casting)

##### Submatrix casting square matrices [#](#submatrix-casting-square-matrices)
...
##### Casting non-square matrices [#](#casting-non-square-matrices)
...
#### Technical Information of `matrix` Types [#](#technical-information-of-matrix-types)
...
Currently, matrices are limited to a maximum of 64 elements (rows\*columns), and a minimum of 1 element. This is because matrices are stored as values (not a reference type), and thus operations on them will be stored on the stack. Restricting the maximum element count minimizes the possibility of stack overflows.
...
## `raw_data` procedure [#](#raw_data-procedure)
...
### Using statement with structs [#](#using-statement-with-structs)
...
## `or_else` expression [#](#or_else-expression)
...
## `or_return` operator [#](#or_return-operator)
...
## `or_continue` operator [#](#or_continue-operator)
...
## `or_break` operator [#](#or_break-operator)
...
### Command-line defines [#](#command-line-defines)
...
#### Advanced Build Tags [#](#advanced-build-tags)
...
## Implicit context system [#](#implicit-context-system)
...
#### Explicit `context` Definition [#](#explicit-context-definition)
...
## Using a `vendor` library [#](#using-a-vendor-library)
...
### Explicit parametric polymorphism [#](#explicit-parametric-polymorphism)
...
#### Procedures using explicit parametric polymorphism (parapoly) [#](#procedures-using-explicit-parametric-polymorphism-parapoly)
...
#### Data types using explicit parametric polymorphism (parapoly) [#](#data-types-using-explicit-parametric-polymorphism-parapoly)
...
### Implicit parametric polymorphism [#](#implicit-parametric-polymorphism)
...
#### Procedures using implicit parametric polymorphism (parapoly) [#](#procedures-using-implicit-parametric-polymorphism-parapoly)
...
## `->` operator (selector call expressions) [#](#--operator-selector-call-expressions)
...
#### `@(default_calling_convention=<string>)` [#](#default_calling_conventionstring)
...
#### `@(deferred_*=<procedure>)` [#](#deferred_procedure)
...
### Record memory layout [#](#record-memory-layout)
...
#### `#caller_expression` or `#caller_expression(<param>)` [#](#caller_expression-or-caller_expressionparam)
...
#### `#force_inline` and `#force_no_inline` [#](#force_inline-and-force_no_inline)
...
#### `#bounds_check` and `#no_bounds_check` [#](#bounds_check-and-no_bounds_check)
...
#### `#type_assert` and `#no_type_assert` [#](#type_assert-and-no_type_assert)
...
#### `#config(<identifier>, default)` [#](#configidentifier-default)
...
#### `#file`, `#directory`, `#line`, `#procedure` [#](#file-directory-line-procedure)
...
#### `#exists(<string-path>)` [#](#existsstring-path)
...
#### `#location()` or `#location(<entity>)` [#](#location-or-locationentity)
...
#### `#load(<string-path>)` or `#load(<string-path>, <type>)` [#](#loadstring-path-or-loadstring-path-type)
...
#### `#hash(<string-text>, <string-hash>)` [#](#hashstring-text-string-hash)
...
#### `#load_hash(<string-path>, <string-hash>)` [#](#load_hashstring-path-string-hash)
...
#### `#load_directory(<string-path>)` [#](#load_directorystring-path)
...
#### If-statements with initialization [#](#if-statements-with-initialization)
...
#### Iterating through slices of structs by value or by reference [#](#iterating-through-slices-of-structs-by-value-or-by-reference)
...
`Maybe(T)` is a union which _either_ returns a type `T` or `nil`. In other languages this is often seen as `Option(T)`, `Result(T)`, etc.
...
## What to do if you get stuck [#](#what-to-do-if-you-get-stuck)
