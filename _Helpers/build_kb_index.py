...
Durable script: re-run after every KB update (new file in `odin-knowledge-base/`, `docs/karl_zylinski/odin-book/`, etc.) to keep the index up to date.
...
def build_source_overview() -> str:
    ...
    lines = [
        ...
        f"| `docs/karl_zylinski/odin-book/` | {karl_count} | Karl's book **'Understanding the Odin Programming Language'** split into 1 file per chapter | Language reference, pure concept, minimal example |",
        f"| `docs/karl_zylinski/*.md` | {zyl_count} | Karl Zylinski blog articles | Game dev patterns, opinions, hot take |",
        f"| `docs/official/` | {off_count} | Official odin-lang.org docs + awesome-odin | Official language reference |",
