#!/usr/bin/env python3
"""
_Helpers/scrape-zylinski.py - Blog Scraper (karl zylinski)
[Re-entrant: skip if <slug>.md already exists; --force to rewrite]

Fetches ALL articles from the https://zylinski.se blog and converts them into Markdown under docs/karl_zylinski/.

Usage:
    python _Helpers/scrape-zylinski.py            # skip if already exported (re-entrant)
    python _Helpers/scrape-zylinski.py --force    # force full rewrite
...
Output:
    docs/karl_zylinski/<slug>.md (one file per article, with footer ">Source: <url>")
...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
OUT = Path(__file__).resolve().parent.parent / "docs" / "karl_zylinski"
