#!/usr/bin/env python3
"""
_Helpers/download_gists.py - Download public GitHub gists from awesome-odin.md.

Parses docs/official/awesome-odin.md to find all gist.github.com URLs, downloads each gist via the public GitHub API, and saves the raw content to code/gists/{gist_id}.odin (or .md for markdown files).

Re-entrant: skips already-downloaded gists unless --force.
--check: dry-run, lists gists that would be downloaded, exits 0.

Usage:
    python _Helpers/download_gists.py             # download missing gists
    python _Helpers/download_gists.py --check     # dry-run: list what would be downloaded
    python _Helpers/download_gists.py --force     # re-download all gists
"""
...
AWESOME_MD = REPO_ROOT / "docs" / "official" / "awesome-odin.md"
