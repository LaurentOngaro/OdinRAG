...
try:
    ...
    raise ImportError("vaultConfigOdinRAG requires PyYAML. Install with: pip install pyyaml") from exc
...
# --- IGNORE LISTS ---
IGNORE_DIRS: set[str] = {
    ".git", ".vscode", ".obsidian", ".kilo", ".agents", ".zed", "__pycache__", "node_modules", ".pytest_cache", "logs", "build", "out", "_archives",
    ".private", "_Private", "_Raw", "_History", "_Tools", "_Refs", "Library", "Temp", "obj", "Builds", "Build", "UserSettings", "code",
    "odin-knowledge-base",
}

IGNORE_FILES: set[str] = {"README.md", ".gitignore", ".gitattributes", "kilocode.json", "copilot-instructions.md", "AGENTS.md"}

VALIDATION_EXCLUDE_DIRS: set[str] = {
    "code/vendored templates", "code/gists", "code/examples", "_Helpers/_obsoletes", "_Private", "odin-knowledge-base/courses",
    "odin-knowledge-base/docs/karl_zylinski/odin-book",
}

VALIDATION_IGNORE_PATTERNS: list[str] = [r"(?i)^readme", r"(?i)index", ]

BUILTIN_BASENAME_EXCLUSIONS: tuple[str, ...] = (r"(?i)^readme", r"(?i)index", )

# --- FRONTMATTER RULES (mirror `_Helpers/docs/003_yaml_frontmatter_conventions.md`) ---
REQUIRED_FIELDS: list[str] = ["title", "date", "tags", "type", "status", "version", "lastUpdated", "updatedBy", ]

OPTIONAL_FIELDS: list[str] = ["docId", "priority", ]

ALLOWED_TYPES: set[str] = {"note", "daily", "weekly", "monthly", "roadmap", "reference", "dev_log", "archive", "template", }

ALLOWED_STATUS: set[str] = {"draft", "active", "done", "archived", "deprecated", }
...
def validate_frontmatter(fm: dict[str, Any] | None) -> list[str]:
    ...
    if "type" in fm and isinstance(fm["type"], str) and fm["type"] not in ALLOWED_TYPES:
        errors.append(f"Invalid type: '{fm['type']}' (allowed: {', '.join(sorted(ALLOWED_TYPES))})")

    if "status" in fm and isinstance(fm["status"], str) and fm["status"] not in ALLOWED_STATUS:
        errors.append(f"Invalid status: '{fm['status']}' (allowed: {', '.join(sorted(ALLOWED_STATUS))})")
    ...
    if "docId" in fm and isinstance(fm["docId"], str) and fm["docId"] and not DOCID_PATTERN.match(fm["docId"]):
        errors.append(f"Invalid docId format: '{fm['docId']}' (expected UPPER_SNAKE_CASE, e.g. NAV_FM_001)")
            ...
            for tag in tag_strs:
                ...
                if not TAG_HIERARCHY_PATTERN.match(tag):
                    if "/" in tag and tag.count("/") >= 2:
                        errors.append(f"Tag '{tag}' has more than 2 hierarchical levels (max: root/sub)")
                    elif " " in tag:
                        errors.append(f"Tag '{tag}' contains spaces (forbidden)")
                    else:
                        errors.append(f"Tag '{tag}' does not match expected pattern (lowercase, optional one '/' sub)")
...
__all__ = [
    "DIAGNOSTIC_DIR", "SCRIPTS_DIR", "HELPERS_DIR", "REPO_ROOT", "KB_ROOT", "CODE_ROOT", "IGNORE_DIRS", "IGNORE_FILES", "VALIDATION_EXCLUDE_DIRS",
    "VALIDATION_IGNORE_PATTERNS", "REQUIRED_FIELDS", "OPTIONAL_FIELDS", "ALLOWED_TYPES", "ALLOWED_STATUS", "ALLOWED_PRIORITY", "REQUIRED_ROOT_TAG",
    "MAX_RAG_FILE_SIZE", "DATE_PATTERN", "VERSION_PATTERN", "DOCID_PATTERN", "TAG_HIERARCHY_PATTERN", "LONG_DASH_PATTERN", "LONG_DASH_CHARS",
    "LONG_DASH_REPLACEMENT", "CJK_PATTERN", "FRONTMATTER_RE", "should_ignore", "should_ignore_file", "should_ignore_dir", "should_validate",
    "extract_frontmatter", "read_file_with_fm", "validate_frontmatter", "path_link", "cprint",
]
