#!/usr/bin/env python3
"""
_Helpers/scripts/fixes/format_odin_in_files.py - Format .odin files and ```odin ... ``` blocks in Markdown files within a given folder.

Wraps `odinfmt` (https://github.com/DanielGavin/ols), the official Odin language formatter (reads stdin, writes stdout).

Usage:
    - python _Helpers/scripts/fixes/format_odin_in_files.py --path DIR          # recursive
    - python _Helpers/scripts/fixes/format_odin_in_files.py --path DIR --check  # dry-run
    - python _Helpers/scripts/fixes/format_odin_in_files.py --file FILE         # single file
...
# Allow importing odin_format from the same folder.
