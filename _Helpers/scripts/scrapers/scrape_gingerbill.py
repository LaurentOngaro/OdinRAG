#!/usr/bin/env python3
"""
_Helpers/scripts/scrapers/scrape_gingerbill.py - Scrape gingerbill.org articles via RSS.
[Re-entrant: skip if <slug>.md already exists; --force to rewrite]

Fetches ALL articles from the https://www.gingerbill.org/article/ RSS feed and converts them into Markdown under odin-knowledge-base/docs/gingerbill/.

Usage:
    python _Helpers/scripts/scrapers/scrape_gingerbill.py            # skip if already exported (re-entrant)
    python _Helpers/scripts/scrapers/scrape_gingerbill.py --force    # force full rewrite
    python _Helpers/scripts/scrapers/scrape_gingerbill.py --check    # dry-run, list what would be scraped
...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
OUT = Path(__file__).resolve().parents[3] / "odin-knowledge-base" / "docs" / "gingerbill"
...
def generate_readme(articles: list[Article]) -> None:
    """Write or update odin-knowledge-base/docs/gingerbill/README.md index."""
    ...
    lines = [
        ...
        "",
        "   ```bash",
        "   python _Helpers/scripts/scrapers/scrape_gingerbill.py            # re-entrant: skip already scraped",
        "   python _Helpers/scripts/scrapers/scrape_gingerbill.py --force    # force re-write",
        "   ```",
        "",
