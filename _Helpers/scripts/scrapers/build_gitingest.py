...
def write_sidecar(entry: RepoEntry, gitingest_version: str, src_mtime: datetime | None) -> None:
    ...
    entry.sidecar_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", )
...
# ─── One pass ────────────────────────────────────────────────────────────────
def prepend_metadata_header(out_path: Path, src: Path, src_mtime: datetime | None, gitingest_version: str) -> None:
    """Prepend a visible metadata block to the snapshot so its freshness is obvious.

    The gitingest output starts with `Directory structure:`. We insert a small
    comment block before it so anyone opening the .txt sees when it was generated,
    which source it mirrors, and the gitingest version used.
    Idempotent: if the header is already present (same `generated_at`), skip.
    """
    if not out_path.exists():
        return
    generated_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    src_mtime_str = src_mtime.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ") if src_mtime else "unknown"
    header_lines = [
        "<!-- gitingest snapshot metadata (do not edit by hand, regenerated by build_gitingest.py) -->", f"# source       : {src}",
        f"# generated_at : {generated_at}", f"# src_mtime    : {src_mtime_str}", f"# gitingest    : {gitingest_version}", "<!-- end metadata -->", "",
    ]
    header = "\n".join(header_lines)
    try:
        ...
        return
    if f"# generated_at : {generated_at}" in text:
        return
    if text.startswith("<!-- gitingest snapshot metadata"):
        return
    out_path.write_text(header + text, encoding="utf-8")


def process_entry(entry: RepoEntry, *, force: bool, check_only: bool, gitingest_version: str, ) -> str:
    ...
    prepend_metadata_header(entry.out_path, src, src_mtime, gitingest_version)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--only", nargs="+", default=[], help="Restrict to entries whose id matches one of the given slugs.", )
    parser.add_argument("--include-skipped", action="store_true", help="Include entries marked skip:true in the JSONC config.", )
    ...
    for entry in entries:
        ...
        result = process_entry(entry, force=args.force, check_only=args.check, gitingest_version=gitingest_version, )
        counts[result] += 1
