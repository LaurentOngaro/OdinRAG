#!/usr/bin/env python3
"""
_Helpers/scripts/scrapers/scrape_newsletters.py - Newsletter Scraper (odin-lang.org/news/)
...
Usage:
    python _Helpers/scripts/scrapers/scrape_newsletters.py            # skip if already exported (re-entrant)
    python _Helpers/scripts/scrapers/scrape_newsletters.py --force    # force full rewrite
    python _Helpers/scripts/scrapers/scrape_newsletters.py --check    # dry-run (report only, no writes)
...
def ensure_readme_updated() -> None:
    ...
    if readme_path.exists():
        ...
        if "scrape-newsletters.py" not in content and marker in content:
            note = (
                "\nRun `python _Helpers/scripts/scrapers/scrape_newsletters.py` to scrape all newsletters "
