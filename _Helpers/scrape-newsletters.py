#!/usr/bin/env python3
"""
_Helpers/scrape-newsletters.py - Newsletter Scraper (odin-lang.org/news/)
[Re-entrant: skip if <slug>.md already exists; --force to rewrite]

Fetches ALL Odin newsletters from the https://odin-lang.org/news/ index and converts them into Markdown under docs/newsletters/.

Usage:
    python _Helpers/scrape-newsletters.py            # skip if already exported (re-entrant)
    python _Helpers/scrape-newsletters.py --force    # force full rewrite
    python _Helpers/scrape-newsletters.py --check    # dry-run (report only, no writes)
...
Output:
    docs/newsletters/<slug>.md  (one file per newsletter, with footer ">Source: <url>")
    docs/newsletters/README.md   (updated with scraper info if needed)
...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
OUT = Path(__file__).resolve().parent.parent / "docs" / "newsletters"
...
def ensure_readme_updated() -> None:
    """Add scraper usage note to docs/newsletters/README.md if missing."""
    ...
    if readme_path.exists():
        ...
        if "scrape-newsletters.py" not in content and marker in content:
            note = (
                "\nRun `python _Helpers/scrape-newsletters.py` to scrape all newsletters "
            ...
            print("  [+] Updated docs/newsletters/README.md with scraper info")
